                ...
                if accumulated_iter % logger_iter_interval == 0 or cur_it == start_it or cur_it + 1 == total_it_each_epoch:
                    ...
                    for name, param in model.named_parameters():
                        if param.grad is not None:
                            ...
                            grad_norm += norm ** 2
                            if gmax > max_grad_value:
                                max_grad_value = gmax
                                max_grad_param_name = name
                            if gmin < min_grad_value:
                                min_grad_value = gmin
                                min_grad_param_name = name
                    ...
                    logger.info(
                        ...
                        'LR: {lr:.3e}  '
                        f'Grad: {grad_norm:.4f}  '
                        f'max={max_grad_value:.4f}({max_grad_param_name})  '
                        f'min: {min_grad_value:.4f}({min_grad_param_name})  '
                        f'NaN: {grad_nan_flag}  '
                        f'{disp_str}  '
