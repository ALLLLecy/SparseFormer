...
from ..model_utils.sparse_utils import SPEncoder
...
class FSHNet_nusc(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, class_names, voxel_size, point_cloud_range, **kwargs):
        ...
        self.down1 = spconv.SparseSequential(
            post_act_block(dim, dim, 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='spconv2', conv_type='spconv'),
            SparseBasicBlock(dim, dim, norm_fn=norm_fn, indice_key='down1'),
        )

        self.down2 = spconv.SparseSequential(
            post_act_block(dim, dim, 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='spconv3', conv_type='spconv'),
            SparseBasicBlock(dim, dim, norm_fn=norm_fn, indice_key='down2'),
        )
        self.down3 = spconv.SparseSequential(
            post_act_block(dim, dim, 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='spconv4', conv_type='spconv'),
            SparseBasicBlock(dim, dim, norm_fn=norm_fn, indice_key='down3'),
        )

        self.stage = SPEncoder(model_cfg.SPENCODER, input_channel=dim)
    ...
    def forward(self, batch_dict):
        ...
        F4, F5, F6 = self.stage([F4, F5, F6])
        F5.indices[:, 1:] *= 2
        F6.indices[:, 1:] *= 4
        F4 = F4.replace_feature(torch.cat([F4.features, F5.features, F6.features]))
        F4.indices = torch.cat([F4.indices, F5.indices, F6.indices])
